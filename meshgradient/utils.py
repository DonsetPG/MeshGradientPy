...
def get_area_from_points(mesh: meshio.Mesh, points: List[int], min_value: float = 1e-10) -> float:
    ...
    Raises:
    """
    a: np.ndarray; b: np.ndarray; c: np.ndarray = mesh.points[points[0]], mesh.points[points[1]], mesh.points[points[2]]
...
def get_cycle(mesh: meshio.Mesh, indx_node: int) -> Tuple[List[Tuple[int]],bool]:
    ...
    unique_indx: np.ndarray; counts_indx: np.ndarray = np.unique(indx_triangle.flatten(), return_counts=True)
    list_of_triangles: List[Tuple[int]] = []
    
    cnt: int; prev: int; curr: int; next: int
    flag_b: bool
    curr_triangle: np.ndarray; next_triangle: np.ndarray
    mask: np.ndarray
